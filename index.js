...
fetch(apiUrl)
  ...
  function createItemCards(data) {
    const clothingSection = document.getElementById("clothingCards");
    const accessoriesSection = document.getElementById("accessoriesCards");
    
    for (let i =0; i < data.length; i++) {
      const card = createItemCard(data[i].id, data[i].preview, data[i].name, data[i].brand, data[i].price);
    ...
    $(document).ready(function () {
        var owl = $("#banner").owlCarousel({
            ...
            onTranslated: function (event) {
                ...
                var dotIndex = currentIndex % 4;
                console.log(dotIndex);
                $(".owl-dot").removeClass("active");
                $(".owl-dot").eq(dotIndex).addClass("active");
            }
        });
        ...
        $(".owl-dot").click(function () {
            ...
            owl.trigger('to.owl.carousel', [dotIndex % 4, 300]);
        });

        $(".owl-nav").click(function () {
          var targetSlide = owl.find(".owl-item.active").index();
          owl.trigger('to.owl.carousel', [targetSlide, 300]);
        });
    });
